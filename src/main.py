...
class TwitterAgent:
    def __init__(self):
        ...
        self.tweet_tracker = TweetTracker(default_comment_limit=3)
        ...
        while True:
            async with self.task_lock:
                ...
                try:
                    ...
                    self.comment_collector.schedule_all_competitor_comments(max_total_tweets=5)
                    self.comment_collector.transfer_top_competitor_comments(num_posts=6)
    ...
    def process_mentions_sync(self):
        """
        Synchronous method to check for mentions every 90 seconds.
        ...
        self.mention_thread_running = True
        logger.info("Starting dedicated mention processing thread...")

        while self.mention_thread_running:
            start_time = time.time()

            try:
                # Create and run a coroutine for processing mentions
                ...
                loop.run_until_complete(process_single_mention())

            except Exception as e:
                logger.error(f"Error checking mentions: {e}")

            # Calculate time to sleep to maintain 90-second interval
            elapsed = time.time() - start_time
            sleep_time = max(0, 120)
            logger.info(f"Mention check took {elapsed:.2f} seconds, sleeping for {sleep_time:.2f} seconds")
            time.sleep(sleep_time)  # Regular sleep, not asyncio.sleep since we're in a sync function

    def start_mention_thread(self):
        """Start the mention processing in a separate thread."""
        if self.mention_thread is not None and self.mention_thread.is_alive():
            logger.info("Mention thread is already running.")
            return

        self.mention_thread = threading.Thread(target=self.process_mentions_sync)
        self.mention_thread.daemon = True  # Thread will exit when main thread exits
        self.mention_thread.start()
        logger.info("Mention thread started.")

    def stop_mention_thread(self):
        """Stop the mention processing thread."""
        if self.mention_thread is None or not self.mention_thread.is_alive():
            logger.info("No mention thread is running.")
            return

        self.mention_thread_running = False
        self.mention_thread.join(timeout=5)
        logger.info("Mention thread stopped.")
    ...
    async def scheduled_posts_comments(self):
        ...
        while True:
            try:
                ...
                logger.error(f"Error in post cycle: {error}")
    ...
    async def run(self):
        ...
        try:
            self.start_mention_thread()

            # Create tasks for other operations
            tasks = [
                asyncio.create_task(self.reset_daily_posts()),
                asyncio.create_task(self.run_tweet_pipeline_daily()),
                asyncio.create_task(self.scheduled_posts_comments()),
            ...
            # Stop the mention thread when shutting down
            self.stop_mention_thread()
